...
2. **`index.html` (portada)**: hero recortado con una silueta orgánica propia (arco asimétrico en la esquina inferior derecha, vía `clip-path` SVG con `objectBoundingBox`, una curva para móvil y otra más amplia para escritorio), con cuatro fotos reales del portfolio rotando en crossfade (`nail-12`, `nail-26`, `nail-15`, `nail-28`; cada 4,5s, se detiene con `prefers-reduced-motion`). El texto —etiqueta de ubicación, titular, subtítulo de dos líneas, los dos CTA y una línea de valoración (4,8/631 reseñas)— va en blanco sobre un velo radial oscuro sutil anclado a la esquina superior izquierda, para que se lea igual de bien con cualquiera de las fotos activas. Debajo: banner en movimiento, franja de cifras reales, presentación ampliada "quiénes somos", cuadrícula "Explora", tira de galería, reseñas reales y CTA final.
